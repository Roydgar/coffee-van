...
public class EditCoffee implements Command{
    ...
    public String execute(HttpServletRequest request) {
        ...
        if (name == null) {
            coffeeId = Integer.parseInt(request.getParameter("id"));
            return EDIT_COFFEE_PAGE;
        }

        CoffeeDaoService.update(new CoffeeBuilder().setId(coffeeId).setName(name).setWeight(Integer.parseInt(weight))
